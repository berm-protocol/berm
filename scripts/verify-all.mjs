...
const GROUPS = [
  ...
  { name: 'vectors', dir: '.', why: 'the frozen vectors still regenerate byte-identically', steps: [
  ...
  { name: 'editor', dir: 'editor', browser: true, why: 'decline path publishes nothing', steps: [
    ['npm', ['ci', '--no-audit', '--no-fund']],
    ['npm', ['run', 'build']],
    ['npm', ['run', 'verify']],
  ]},
  { name: 'post', dir: 'post', why: 'X unit counting, refuse-not-truncate, no claim of X delivery', steps: [
    ['npm', ['ci', '--no-audit', '--no-fund']],
    ['npm', ['run', 'typecheck']],
    ['npm', ['test']],
  ]},
  { name: 'post-browser', dir: 'post', browser: true, why: 'card rasterises, decline publishes nothing, intent opens', steps: [
    ['npm', ['run', 'build']],
    ['npm', ['run', 'verify']],
  ]},
  ...
  { name: 'node-pages', dir: 'node-pages', why: 'a node publishes only what it verified, and refuses to truncate', steps: [
    ['npm', ['ci', '--no-audit', '--no-fund']],
    ['npm', ['run', 'typecheck']],
    ['npm', ['test']],
    ['npm', ['run', 'verify']],
  ...
  { name: 'link', dir: 'link', browser: true, why: 'claim never renders as verified', steps: [
    ['npm', ['ci', '--no-audit', '--no-fund']],
    ['npm', ['run', 'build']],
    ['npm', ['run', 'verify']],
  ]},
  { name: 'recovery', dir: 'recovery', browser: true, why: 'readiness verdicts and guardian prompt', steps: [
    ['npm', ['ci', '--no-audit', '--no-fund']],
    ['npm', ['run', 'build']],
    ['npm', ['run', 'verify']],
  ]},
  ...
  { name: 'supply-chain', dir: '.', why: 'no CDN, no quarantined code in a shipped artifact, CI runs every group', steps: [
